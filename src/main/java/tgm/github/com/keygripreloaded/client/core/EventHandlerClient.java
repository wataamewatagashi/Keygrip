...
{
    public final ResourceLocation txRec = new ResourceLocation("keygripreloaded", "textures/icon/recording.png");
    public final ResourceLocation txRecPulse = new ResourceLocation("keygripreloaded", "textures/icon/recording_pulse.png");
    ...
    public void onKeyEvent(KeyEvent event)
    {
        ...
        if(event.keyBind.equals(KeygripReloaded.config.toggleSceneRecorder))
        {
        ...
        else if(KeygripReloaded.eventHandlerClient.workspace != null)
        {
            ...
            else if(event.keyBind.equals(KeygripReloaded.config.toggleScenePlayback) && KeygripReloaded.eventHandlerClient.workspace.hasOpenScene())
            {
                if(KeygripReloaded.eventHandlerClient.workspace.getOpenScene().playing)
                {
                    ...
                    KeygripReloaded.channel.sendToServer(new PacketStopScene(KeygripReloaded.eventHandlerClient.workspace.getOpenScene().identifier));
                    if (KeygripReloaded.eventHandlerClient.actionToRecord != null) {
                        KeygripReloaded.eventHandlerClient.workspace.toggleRecording();
                    }
                } else {
                    if(KeygripReloaded.eventHandlerClient.workspace.sceneSendingCooldown <= 0) {
                        KeygripReloaded.eventHandlerClient.workspace.timeline.timeline.setCurrentPos(0);
                        if(GuiScreen.isCtrlKeyDown())
                        {
                            Minecraft.getMinecraft().displayGuiScreen(null);
                            Minecraft.getMinecraft().setIngameFocus();
                        }
                        Scene.sendSceneToServer(KeygripReloaded.eventHandlerClient.workspace.getOpenScene());
                    }
                    KeygripReloaded.eventHandlerClient.workspace.sceneSendingCooldown = 10;
                }
            }
        }
    }
    ...
    public void onWorldTick(TickEvent.ClientTickEvent event)
    {
        ...
        if(event.phase == TickEvent.Phase.START)
        {
            ...
            if(actionToRecord != null && !mc.isGamePaused() && startRecord && (KeygripReloaded.config.playbackSceneWhileRecording != 1 || sceneFrom.playTime >= actionToRecord.startKey + startRecordTime))
            {
