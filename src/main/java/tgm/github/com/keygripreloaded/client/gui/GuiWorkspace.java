package tgm.github.com.keygripreloaded.client.gui;
...
import tgm.github.com.keygripreloaded.client.gui.window.WindowSaveAs;
import tgm.github.com.keygripreloaded.client.gui.window.WindowSceneSelection;
import tgm.github.com.keygripreloaded.client.gui.window.WindowTimeline;
import tgm.github.com.keygripreloaded.client.gui.window.WindowTopDock;
...
import tgm.github.com.keygripreloaded.common.scene.action.ActionComponent;
import tgm.github.com.keygripreloaded.common.scene.action.EntityState;
...
public class GuiWorkspace extends IWorkspace
{
    ...
    public void drawScreen(int mouseX, int mouseY, float renderTick)
    {
        ...
        EventHandlerClient eventHandler = KeygripReloaded.eventHandlerClient;
    ...
    public void toggleRecording()
    {
        ...
        {
            if(KeygripReloaded.config.playbackSceneWhileRecording == 1 && KeygripReloaded.eventHandlerClient.sceneFrom != null)
            {
                KeygripReloaded.eventHandlerClient.sceneFrom.stop();
                KeygripReloaded.channel.sendToServer(new PacketStopScene(KeygripReloaded.eventHandlerClient.sceneFrom.identifier));
            }
            KeygripReloaded.eventHandlerClient.actionToRecord.actionComponents.put(KeygripReloaded.eventHandlerClient.recordActionFrom, new ArrayList<ActionComponent>()
            {{
                add(new ActionComponent(0, 0, null));
            }});
            KeygripReloaded.eventHandlerClient.actionToRecord = null;
            KeygripReloaded.eventHandlerClient.sceneFrom = null;
            ...
            for(Action a : scene.actions)
            {
                ...
                started = true;
                KeygripReloaded.eventHandlerClient.sceneFrom = scene;
                KeygripReloaded.eventHandlerClient.actionToRecord = a;
                KeygripReloaded.eventHandlerClient.recordActionFrom = Math.max(Math.min(a.getLength(), timeline.timeline.getCurrentPos() - a.startKey), 0);
                KeygripReloaded.eventHandlerClient.startRecordTime = KeygripReloaded.eventHandlerClient.recordActionFrom;
                ...
                if(KeygripReloaded.eventHandlerClient.recordActionFrom == 0)
                {
                    KeygripReloaded.eventHandlerClient.actionToRecord.offsetPos = new int[] { (int)Math.round(mc.player.posX * Scene.PRECISION) - scene.startPos[0], (int)Math.round(mc.player.posY * Scene.PRECISION) - scene.startPos[1], (int)Math.round(mc.player.posZ * Scene.PRECISION) - scene.startPos[2] };
                    KeygripReloaded.eventHandlerClient.actionToRecord.rotation = new int[] {Math.round(mc.player.rotationYaw * Scene.PRECISION), Math.round(mc.player.rotationPitch * Scene.PRECISION)};
                    ...
                    for(Map.Entry<Integer, LimbComponent> e : a.lookComponents.entrySet()) {
                        if(e.getKey() > lastLookInt && e.getKey() < KeygripReloaded.eventHandlerClient.recordActionFrom) {
                    ...
                    {
                        if(e.getKey() > lastPosInt && e.getKey() < KeygripReloaded.eventHandlerClient.recordActionFrom)
                        {
                            lastPosInt = e.getKey();
                            lastPos = e.getValue();
                            }
                ...
                a.actionComponents.entrySet().removeIf(e -> e.getKey() >= KeygripReloaded.eventHandlerClient.recordActionFrom);
                a.posComponents.entrySet().removeIf(e -> e.getKey() >= KeygripReloaded.eventHandlerClient.recordActionFrom);
                break;
            }
