...
public class WindowEditAction extends Window
{
    ...
    public WindowEditAction(IWorkspace parent, int x, int y, int w, int h, int minW, int minH) {
        ...
        Class<? extends Entity> clz = EntityPlayer.class;
        if (action != null && action.entityType != null && gWorkspace.containsEntity.containsKey(Splitter.on(".").splitToList(action.entityType).get(Splitter.on(".").splitToList(action.entityType).size() - 1))) {
            clz = gWorkspace.containsEntity.get(Splitter.on(".").splitToList(action.entityType).get(Splitter.on(".").splitToList(action.entityType).size() - 1));
        }

        elements.add(elementEntitySelector = new ElementEntitySelector(this, 10, 65, width -20, 13, -3, clz));
